...
system-specific and follow the SQL
[`HINT` syntax][hints].
...
- It's data type can be coerced into the corresponding table element
  column type.
- If the column is an `BIGINT` type and the corresponding table element is an
  `INTEGER` type.
- If the column is an `UBIGINT` type and the corresponding table element is an
  `UINTEGER` type.
...
```sql
CREATE TABLE library.books (title VARCHAR, author VARCHAR);
```

Create a table that contains schema options.

```sql
CREATE TABLE books (title VARCHAR, author VARCHAR) OPTIONS (storage_kind=FLASH_MEMORY);
```

Partition a table.

```sql
CREATE TABLE books (title VARCHAR, author VARCHAR, publisher VARCHAR, release_date DATE)
...
column_definition:
  ...
  {
    column_type
    | [ column_type ] generated_column_clause
  }
  [ column_attribute[ ... ] ]
  [ OPTIONS (...) ]
...
- `column_name`: The name of the column. The name of a column must be unique
  within a table.
- `column_type`: The SQL data type of the column.
...
```sql
CREATE TABLE books (title VARCHAR, author VARCHAR, isbn BIGINT PRIMARY KEY NOT NULL);
```
...
CREATE TABLE Authors (
  first_name VARCHAR HIDDEN,
  last_name VARCHAR HIDDEN,
  full_name VARCHAR GENERATED ALWAYS AS (CONCAT(first_name, ' ', last_name)));
```
...
CREATE TABLE books (
  ...
  category VARCHAR OPTIONS (description="LCC Subclass")
)
```
...
```sql
CREATE TABLE books (title VARCHAR, author VARCHAR, PRIMARY KEY (title ASC, author ASC));
```
...
CREATE TABLE page_count_average (
  words_per_chapter BIGINT,
  words_per_book BIGINT,
...
- `MATCH`: Specifies when a constraint validation for a referencing row
  ...
  - `SIMPLE`:
    - Passes if _any column_ of the local key is `NULL`.
  ...
  - `FULL`:
    - Passes if _all columns_ of the local key are `NULL`.
    - Passes if the columns of the local key are pairwise-equal to the
      columns of the referenced key for some row of the referenced table.
  - `NOT DISTINCT`:
...
[defining-constraints]: #defining-table_constraints
[defining-foreign-reference]: #defining-foreign_references
[generated-column-clause]: #generated-column_clause
[primary-key]: #primary-key
[assignable-types]: #assignable-types
...
[udas]: ../functions/user_defined_aggregates.md#udas
...
[udfs]: ../functions/user_defined_functions.md
...
- `OPTIONS`: If you have schema options, you can add them when you create the
  procedure. These options are system specific and follow the SQL[`HINT`
  syntax][hints].
...
```sql
CREATE PROCEDURE my_procedure(IN x BIGINT, OUT y VARCHAR)
BEGIN
  IF x > 0 THEN
    SET y = 'positive';
  ELSE
...
[tvfs]: ../functions/table_functions.md#tvfs
...
The option entries are system-specific. These follow the SQL
[`HINT` syntax][hints].
...
```sql
CREATE TABLE books (title VARCHAR, id VARCHAR, PRIMARY KEY (id));
```
...
```sql
CREATE TABLE books (title VARCHAR, name VARCHAR, PRIMARY KEY (title, name));
```
