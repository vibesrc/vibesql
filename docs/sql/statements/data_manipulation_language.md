...
| ----------- | ----------------- | ------------------ |
| SingerId    | `BIGINT NOT NULL` | `<auto-increment>` |
| FirstName   | `VARCHAR`         |                    |
| LastName    | `VARCHAR`         |                    |
| BirthDate   | `DATE`            |                    |
...
| Column Name  | Data Type         | Default Value      |
| ------------ | ----------------- | ------------------ |
| VenueId      | `BIGINT NOT NULL` | `<auto-increment>` |
| SingerId     | `BIGINT NOT NULL` |                    |
| ConcertDate  | `DATE NOT NULL`   |                    |
| BeginTime    | `TIMESTAMP`       |                    |
| EndTime      | `TIMESTAMP`       |                    |
| TicketPrices | `ARRAY<BIGINT>`   |                    |
...
- The value type matches the column type exactly. (For example, inserting a
  value of type `INTEGER` in a column that also has a type of `INTEGER`.)
...
- The value can be assigned after applying a narrowing integer or floating point
  cast. For example, if you attempt to add a value of `BIGINT` into a column that
  supports `INTEGER`, SQL automatically adds a narrowing cast to
  `INTEGER`.
...
For example, the following statement instructs SQL to skip the
...
primary key. SQL raises an error if any other constraint
...
With SQL, you can confirm how many rows are added each time you
...
expect the command to insert, and SQL compares that number
against the number of rows actually modified. This check occurs before
SQL commits the change to the database. If the row count matches,
SQL commits the changes. Otherwise, SQL returns
an error and rolls back the statement.

When calculating the number of rows modified, SQL includes rows
...
regards to what data SQL should update.
...
With SQL, you can confirm how many rows are deleted each time you
...
actually deleted. This check occurs before SQL commits the change
to the database. If the row count matches, SQL commits the
changes. Otherwise, SQL returns an error and rolls back the
statement.
...
With SQL, you can confirm how many rows are updated each time you
...
With the `ASSERT_ROWS_MODIFIED` keyword, you specify the number of rows you
expect the command to update. If the row count matches, SQL
commits the changes. Otherwise, SQL returns an error and rolls
back the statement.
...
You specify values for specific columns using the syntax _column_name =
expression_. Expressions can see all the columns within a table. For
example:
...
SQL allows you to update non-repeating or repeating fields within
. To illustrate how to update a non-repeating field, consider
the [Singers example table][singers-table]. It contains a column, `Albums`, of
type ``, and `Albums`contains a non-repeating field`tracks`. The
...
a repeated field of a or an array. Such statements are called
**nested updates**.
...
`Albums` also contains a repeated , `Song`, which provides
...
If the repeated field is another , you can provide the
as a string literal. For example, the following statement adds a
...
For example, the `Song` itself has another repeated
, `Chart`, which provides information on what chart the song
...
SQL treats an array or a repeated field inside a row that matches
...
[from-clause]: ../syntax/query_syntax.md#from-clause
...
[from-clause]: ../syntax/query_syntax.md#from-clause
[join-operator]: ../syntax/query_syntax.md#join-types
[dml-then-return-clause]: #dml-then_return_clause
[coercion]: ../types/conversion_rules.md#coercion
[functions-and-operators]: ../functions/functions_reference.md
[statement-rules]: #statement-rules
