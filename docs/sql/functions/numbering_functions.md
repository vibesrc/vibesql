...
SQL supports numbering functions.
...
| Name                                                  | Summary                                                                                 |
| ----------------------------------------------------- | --------------------------------------------------------------------------------------- |
| [`CUME_DIST`](numbering_functions.md#cume-dist)       | Gets the cumulative distribution (relative position (0,1]) of each row within a window. |
| [`DENSE_RANK`](numbering_functions.md#dense-rank)     | Gets the dense rank (1-based, no gaps) of each row within a window.                     |
| [`IS_FIRST`](numbering_functions.md#is-first)         | Returns `true` if this row is in the first `k` rows (1-based) within the window.        |
| [`IS_LAST`](numbering_functions.md#IS-LAST)           | Returns `true` if this row is in the last `k` rows (1-based) within the window.         |
| [`NTILE`](numbering_functions.md#ntile)               | Gets the quantile bucket number (1-based) of each row within a window.                  |
| [`PERCENT_RANK`](numbering_functions.md#percent-rank) | Gets the percentile rank (from 0 to 1) of each row within a window.                     |
| [`RANK`](numbering_functions.md#rank)                 | Gets the rank (1-based) of each row within a window.                                    |
| [`ROW_NUMBER`](numbering_functions.md#row-number)     | Gets the sequential row number (1-based) of each row within a window.                   |
