...
SQL supports the following datetime functions.
...
| [`PARSE_DATETIME`]   | Converts a `VARCHAR` value to a `DATETIME` value.                                                  |
...
[datetime-timezone-definitions]: timestamp_functions.md#timezone-definitions
...
1. Constructs a `DATETIME` object using `BIGINT` values
...
[datetime-trunc-granularity-date]: #datetime-trunc_granularity_date
[datetime-trunc-granularity-time]: #datetime-trunc_granularity_time
[datetime-time-zone-parts]: #datetime-time_zone_parts
...
[datetime-format-elements]: ../types/format_elements.md#format-elements_date_time
...
Converts a `VARCHAR` value to a `DATETIME` value.

**Definitions**

- `format_string`: A `VARCHAR` value that contains the
  [format elements][datetime-format-elements] to use with `datetime_string`.
- `datetime_string`: A `VARCHAR` value that represents the date and time to
  parse.
...
The following examples parse a `VARCHAR` literal as a
`DATETIME`.
...
The following example parses a `VARCHAR` literal
