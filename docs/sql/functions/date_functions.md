...
SQL supports the following date functions.

## Function list

| Name                                                            | Summary                                                                                                  |
| --------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| [`CURRENT_DATE`](date_functions.md#current-date)                | Returns the current date as a `DATE` value.                                                              |
| [`DATE`](date_functions.md#date)                                | Constructs a `DATE` value.                                                                               |
| [`DATE_ADD`](date_functions.md#date-add)                        | Adds a specified time interval to a `DATE` value.                                                        |
| [`DATE_DIFF`](date_functions.md#date-diff)                      | Gets the number of unit boundaries between two `DATE` values at a particular time granularity.           |
| [`DATE_FROM_UNIX_DATE`](date_functions.md#date-from-unix-date)  | Interprets an `BIGINT` expression as the number of days since 1970-01-01.                                |
| [`DATE_SUB`](date_functions.md#date-sub)                        | Subtracts a specified time interval from a `DATE` value.                                                 |
| [`DATE_TRUNC`](date_functions.md#date-trunc)                    | Truncates a `DATE`, `DATETIME`, or `TIMESTAMP` value at a particular granularity.                        |
| [`EXTRACT`](date_functions.md#extract)                          | Extracts part of a date from a `DATE` value.                                                             |
| [`FORMAT_DATE`](date_functions.md#format-date)                  | Formats a `DATE` value according to a specified format string.                                           |
| [`GENERATE_DATE_ARRAY`](array_functions.md#generate-date-array) | Generates an array of dates in a range. For more information, see [Array functions](array_functions.md). |
| [`LAST_DAY`](date_functions.md#last-day)                        | Gets the last day in a specified time period that contains a `DATE` value.                               |
| [`PARSE_DATE`](date_functions.md#parse-date)                    | Converts a `VARCHAR` value to a `DATE` value.                                                            |
| [`UNIX_DATE`](date_functions.md#unix-date)                      | Converts a `DATE` value to the number of days since 1970-01-01.                                          |
...
- `time_zone_expression`: A `VARCHAR` expression that represents a
...
[date-timezone-definitions]: ../types/data_types.md#time-zones
...
- `year`: The `BIGINT` value for year.
- `month`: The `BIGINT` value for month.
- `day`: The `BIGINT` value for day.
...
[date-timezone-definitions]: timestamp_functions.md#timezone-definitions
...
[date-trunc-granularity-date]: #date-trunc_granularity_date
[date-trunc-granularity-time]: #date-trunc_granularity_time
[date-time-zone-parts]: #date-time_zone_parts
...
[date-format-elements]: ../types/format_elements.md#format-elements_date_time
...
Converts a `VARCHAR` value to a `DATE` value.

**Definitions**

- `format_string`: A `VARCHAR` value that contains the
  [format elements][date-format-elements] to use with `date_string`.
- `date_string`: A `VARCHAR` value that represents the date to parse.
